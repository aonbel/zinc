...
template <class Rep, class Period, class ClockT> struct DurationAwaitable {
  ...
  void await_suspend(std::coroutine_handle<PromiseFor<U>> handle) {
    ...
    if (global_thread_pool) {
      global_thread_pool->SubmitAfter(ClockT::now() + duration_,
...
zinc::core::async::awaiters::DurationAwaitable<Rep, Period, ClockT>
operator co_await(const std::chrono::duration<Rep, Period> &duration) {
  return {duration};
}